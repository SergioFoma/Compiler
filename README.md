...
Компилятор написан на C/C++ и состоит из четырех частей:
...
4. ASM: Генерация байт кода. Осуществляется перевод ассемблерных команд в байт-код.
...
>⚠️ Сейчас неинициализированная переменная приводит к неопределенному
> поведению. В будущем я планирую добавить анализ на этапе компиляции,
> чтобы выдавать ошибку, как в безопасных языках.
...
|   Извлечение квадратного арифметического корня  |                    sqrt                             |
